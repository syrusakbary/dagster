...
def dask_tests():
    ...
    for version in SupportedPythons:
        ...
        tests.append(
            ...
            .run(
                "cd python_modules/dagster-dask/dagster_dask_tests/dask-docker",
                "apt-get update",
                ...
                "export CONTAINER_NAME=`docker ps --filter \"id=`cut -c9- < /proc/1/cpuset`\" --format \"{{.Names}}\"`",
                # then, we dynamically bind this container into the dask user-defined bridge
                # network to make the dask containers visible...
                "docker network connect dask ${CONTAINER_NAME}",
