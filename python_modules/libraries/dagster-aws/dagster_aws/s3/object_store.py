...
class S3ObjectStore(ObjectStore):
    def __init__(self, bucket, protocol):
        self.bucket = check.str_param(bucket, 'bucket')
        self.protocol = check.str_param(protocol, 'protocol')
    ...
    def set_object(self, key, obj, serde=None):
        ...
        check.inst_param(serde, 'serde', SerDe)  # cannot be none here
    ...
    def uri_for_key(self, key):
        check.str_param(key, 'key')
        return self.protocol + self.bucket + '/' + '{key}'.format(key=key)
