...
def test_unknown_error():
    ...
    with mock.patch(
        'gevent.pywsgi.WSGIServer', new_callable=_define_mock_server(_raise_custom_error)
    ):
        handle = ExecutionTargetHandle.for_repo_yaml(script_relative_path('./repository.yml'))
...
def test_port_collision():
    ...
    with mock.patch('gevent.pywsgi.WSGIServer', new_callable=_define_mock_server(_raise_os_error)):
        handle = ExecutionTargetHandle.for_repo_yaml(script_relative_path('./repository.yml'))
