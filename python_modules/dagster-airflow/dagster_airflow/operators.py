...
from dagster.seven.json import dumps, JSONDecodeError
    ...
    def make_python_callable(cls, handle, pipeline_name, mode, env_config, step_keys):
        ...
        def python_callable(**kwargs):
            ...
            logging.info(
                ...
                + '\n'
                + 'with variables:\n'
                + dumps(dict(variables, environmentConfigData='REDACTED'))
            )

            res = json.loads(execute_query_from_cli(handle, QUERY, variables=dumps(variables)))
