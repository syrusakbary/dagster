...
class IntermediateStore(six.with_metaclass(ABCMeta)):
    def __init__(self, object_store, root, sep=os.sep, types_to_register=None):
    ...
    def uri_for_paths(self, paths, protocol=None):
        ...
        return self.object_store.uri_for_key(self.key_for_paths(paths), protocol)

    def key_for_paths(self, paths):
        return self.sep.join([self.root] + paths)

    def get_run_files_path(self, root, run_id):
        '''Normalized path structure for dagster runs
        '''
        root = [root] if root else []
        return self.sep.join(root + ['dagster', 'runs', run_id, 'files'])
    ...
    def set_object(self, obj, context, runtime_type, paths):
        ...
        return self.object_store.set_object(key, obj, runtime_type.serde)
    ...
    def get_object(self, context, runtime_type, paths):
        ...
        check.inst_param(runtime_type, 'runtime_type', RuntimeType)
        key = self.key_for_paths(paths)
        return self.object_store.get_object(key, serde=runtime_type.serde)
...
class FileSystemIntermediateStore(IntermediateStore):
    def __init__(self, run_id, types_to_register=None, base_dir=None):
        ...
        super(FileSystemIntermediateStore, self).__init__(
            ...
            root=self.get_run_files_path(self.base_dir, run_id),
            sep=self.sep,
    ...
    def copy_object_from_prev_run(
        ...
        prev_run_files_dir = self.get_run_files_path(self.base_dir, previous_run_id)
