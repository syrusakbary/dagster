...
def test_diamond_graph():
    ...
    @pipeline(solids=[emit_values, add_one, add])
    def diamond_pipeline(graph):
        value_one, value_two = graph.emit_values()
        graph.add(
            num_one=graph.add_one(num=value_one),
            num_two=graph.add_one.alias('renamed')(num=value_two),
        )
