...
lazy val events = project
  .settings(
    ...
    resolvers += Resolver.sonatypeRepo("releases"),
      ...
      // Cloud Dataproc 1.4.x releases all use the 1.6.16 gcs-connector, so we use it here. Also note that there's an
      // issue w/ the transitive dep on jmxtools so we exclude it here.
      //
      // See:
